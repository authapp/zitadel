...
describe('User Address Projection Integration Tests', () => {
  ...
  beforeEach(async () => {
    // Clean up test data
    await pool.query('DELETE FROM user_addresses WHERE instance_id = $1', [TEST_INSTANCE_ID]);
  ...
  describe('Address Changed Events', () => {
    it('should process user.human.address.changed event', async () => {
      ...
      const result = await pool.queryOne(
        'SELECT * FROM user_addresses WHERE instance_id = $1 AND user_id = $2',
    ...
    it('should process user.v1.address.changed event', async () => {
      ...
      expect(result.postal_code).toBe('M5H 2N2');
    ...
    it('should format address correctly', async () => {
      ...
      const result = await pool.queryOne(
        'SELECT formatted_address FROM user_addresses WHERE instance_id = $1 AND user_id = $2',
        [TEST_INSTANCE_ID, userId]
      );

      expect(result.formatted_address).toBe('456 Oak Ave, San Francisco, CA, 94102, USA');
    });
    ...
    it('should update existing address', async () => {
      ...
      expect(result.postal_code).toBe('02139');
    });
    ...
    it('should handle address types', async () => {
      ...
      const result = await pool.queryOne(
        'SELECT address_type, is_primary FROM user_addresses WHERE instance_id = $1 AND user_id = $2',
        [TEST_INSTANCE_ID, userId]
      );
